...
export function BassPianoRoll({ pattern, selected, selectedBars, onSelect, onBars, onChange }: Props) {
  ...
  const duplicate = () => { if (!selected) return; const wasApproach = ['approach', 'chromatic_approach'].includes(selected.harmonic_role); const gridTick = Math.min(total - 1, selected.grid_tick + 240); const copy = { ...selected, event_id: crypto.randomUUID(), grid_tick: gridTick, structural_offset_tick: Math.max(-gridTick, Math.min(total - 1 - gridTick, selected.structural_offset_tick)), harmonic_role: wasApproach ? 'scale_tone' as const : selected.harmonic_role, rhythmic_role: wasApproach ? 'decoration' as const : selected.rhythmic_role, approach_target_id: null, decision_trace: null, provenance: { ...selected.provenance, origin: 'user_edited' } }; onChange({ ...pattern, events: [...pattern.events, copy].sort((a, b) => a.grid_tick - b.grid_tick), analysis: null }); onSelect(copy) }
