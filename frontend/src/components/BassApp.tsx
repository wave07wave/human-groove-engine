...
export function BassApp({ groovePattern, onGrooveUpdate, onBassPatternChange }: { groovePattern: GroovePattern | null, onGrooveUpdate?: (pattern: GroovePattern) => void, onBassPatternChange?: (pattern: BassPattern | null) => void }) {
  ...
  const [preference, setPreference] = useState<BassPreferenceSummary | null>(null)
  ...
  useEffect(() => { setCandidates([]); setJointResults([]) }, [preset])
  ...
  const generate = async () => { if (!intent) return; setBusy(true); setError(''); setEvaluationStatus(''); try {
    ...
    setSelected(null); setSelectedBars(new Set())
