    ...
    <main>
      <section className="control-panel panel">
        <div className="transport-settings">
          ...
          <label>拍子<select value={meter} onChange={e=>setMeter(e.target.value)}>{METER_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></label>
      ...
      {pattern&&<section className="advanced panel"><div className="tabs">{['拍の安定','シンコペーション','楽器の噛み合い','強弱','タイミング','フレーズ','複雑さ','リスナー'].map(tab=><button className={advanced===tab?'active':''} key={tab} onClick={()=>setAdvanced(tab)}>{tab}</button>)}</div><div className="advanced-body"><div><p className="eyebrow">{advanced} / 目標 ↔ 実測</p><h2>目標が導き、パターンが応えます。</h2><p className="muted">実測値は生成されたイベントから算出され、操作値をそのまま複製するものではありません。</p></div><div className="dna-table">{targetVsMeasured.map(([key,value])=><div key={key}><span>{key.replaceAll('_',' ')}</span><i><em style={{width:`${Number(value)*100}%`}}/></i><b>{Math.round(intent.target_dna[key as keyof GrooveDNA]*100)} → {Math.round(Number(value)*100)}</b></div>)}</div></div></section>}
...
export default function App() {
  ...
  return <>
    ...
    {sharedGroove && sharedBass && <aside className="mix-transport" aria-label="GrooveとBassの同時再生"><span>Groove + Bass</span><button className={mixPlaying ? 'play active' : 'play'} onClick={() => { setMixError(''); void toggleMixPreview(sharedGroove, sharedBass, setMixPlaying).catch(cause => setMixError(`音声を開始できません: ${String(cause)}`)) }}>{mixPlaying ? '■ 停止' : '▶ 再生'}</button>{mixError && <small role="alert">{mixError}</small>}</aside>}
