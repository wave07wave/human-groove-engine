...
import { anonymousSessionId, EmbodiedFeedbackPanel } from './components/EmbodiedFeedbackPanel'
...
export function GrooveApp({ onPatternChange, externalPattern }: { onPatternChange?: (pattern: GroovePattern | null) => void, externalPattern?: GroovePattern | null }) {
  ...
  useEffect(() => { if (externalPattern && externalPattern.pattern_id !== pattern?.pattern_id) { evaluationSequence.current += 1; history.commit(externalPattern) } }, [externalPattern, pattern?.pattern_id, history])
  useEffect(() => {
  ...
  const generate = async () => { const sequence = ++evaluationSequence.current; setBusy(true); setError(''); try {
    ...
    setSeed(nextSeed)
    const request: GenerateRequest = { bpm,bars,meter:{...METERS[meter],subdivisions_per_quarter:resolution},intent,preset,seed:nextSeed,mode:'preview',performance_mode:performanceMode,render_profile:renderProfile,candidate_count:4, anonymous_session_id:anonymousSessionId() }
  ...
  const evaluationRequest = useMemo<GenerateRequest>(() => ({ bpm,bars,meter:{...METERS[meter],subdivisions_per_quarter:resolution},intent,preset,seed,mode:'preview',performance_mode:'auto',render_profile:renderProfile,candidate_count:1, anonymous_session_id:anonymousSessionId() }), [bpm,bars,meter,resolution,intent,preset,seed,renderProfile])
  return <div className="app-shell">
    ...
    <main>
      <section className="control-panel panel">
        <div className="transport-settings">
          ...
          <label>スタイル<select value={preset} onChange={e=>choosePreset(e.target.value)}>{Object.keys(presets?.built_in??{Balanced:1}).map(x=><option key={x}>{x}</option>)}</select></label>
    ...
    </main><footer><span>ENGINE 0.10 · 21-DIMENSION QUALITY AUDIT</span><span>Stable pulse → controlled violation → recovery</span></footer>
  </div>
}
